...
- V2 has one staging identity, not roles or SSO. Opt-in mutation POSTs (Phase 2/3/6) require a loopback `Host` (`localhost` / `127.0.0.1` / `::1`) unless `MISSION_CONTROL_ALLOW_REMOTE_MUTATIONS=enabled`. Non-loopback hosts receive `403 MUTATION_HOST_DENIED`. This is the CSRF residual control for browser-exposed staging; full CSRF tokens remain a future hardening step if cookie-based browser auth widens.
